...
const OllamaModelForm = memo<OllamaModelFormProps>(({ id, model }) => {
	...
	const [delAndRegenerateMessage, deleteMessage] = useChatStore((s) => [s.delAndRegenerateMessage, s.deleteMessage]);
	...
	return (
		<Center gap={16} style={{ maxWidth: 300, width: "100%" }}>
			<FormAction
				...
				title={isDownloading ? t("ollama.download.title", { model: modelToPull }) : t("ollama.unlock.title")}>
				{!isDownloading && (
					<Input
						onChange={(e) => {
