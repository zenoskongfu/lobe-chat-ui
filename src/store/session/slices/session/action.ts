...
export interface SessionAction {
	 ...
	 * @returns sessionId
	 */
	createSession: (session?: DeepPartial<LobeAgentSession>, isSwitchSession?: boolean) => Promise<string>;
	...
	internal_processSessions: (sessions: LobeSessions, customGroups: LobeSessionGroups, actions?: string) => void;
	/* eslint-enable */
}
...
export const createSessionSlice: StateCreator<SessionStore, [["zustand/devtools", never]], [], SessionAction> = (
	...
	createSession: async (agent, isSwitchSession = true) => {
		...
		const defaultAgent = merge(DEFAULT_AGENT_LOBE_SESSION, settingsSelectors.defaultAgent(useUserStore.getState()));
	...
	updateSearchKeywords: (keywords) => {
		set({ isSearching: !!keywords, sessionSearchKeywords: keywords }, false, n("updateSearchKeywords"));
	},
	...
	internal_processSessions: (sessions, sessionGroups) => {
		...
		const defaultGroup = sessions.filter((item) => (!item.group || item.group === "default") && !item.pinned);
