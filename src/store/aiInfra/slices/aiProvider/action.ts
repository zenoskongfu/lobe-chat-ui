...
export const createAiProviderSlice: StateCreator<AiInfraStore, [["zustand/devtools", never]], [], AiProviderAction> = (
	set,
	get
) => ({
	...
	internal_toggleAiProviderConfigUpdating: (id, loading) => {
		set(
			(state) => {
				if (loading) return { aiProviderConfigUpdatingIds: [...state.aiProviderConfigUpdatingIds, id] };
			...
			// () => aiProviderService.getAiProviderList(),
			{
				...
				onSuccess: (data) => {
					if (!get().initAiProviderList) {
						set({ aiProviderList: data, initAiProviderList: true }, false, "useFetchAiProviderList/init");
						return;
					}
		...
		useClientDataSWR<AiProviderRuntimeState | undefined>(
			...
			async ([, isLogin]) => {
				...
				return {
					...
					enabledAiProviders: DEFAULT_MODEL_PROVIDER_LIST.filter((provider) => provider.enabled).map(
						(item) => ({ id: item.id, name: item.name, source: "builtin" })
					),
					runtimeConfig: {},
				};
			},
