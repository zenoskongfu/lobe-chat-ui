...
export interface AgentChatAction {
	...
	internal_dispatchAgentMap: (id: string, config: DeepPartial<LobeAgentConfig>, actions?: string) => void;
	internal_refreshAgentConfig: (id: string) => Promise<void>;
	internal_refreshAgentKnowledge: () => Promise<void>;
	internal_updateAgentConfig: (id: string, data: DeepPartial<LobeAgentConfig>, signal?: AbortSignal) => Promise<void>;
...
export const createChatSlice: StateCreator<AgentStore, [["zustand/devtools", never]], [], AgentChatAction> = (
	set,
	get
) => ({
	...
	useFetchAgentConfig: (isLogin, sessionId) =>
		useClientDataSWR<LobeAgentConfig>(
			...
			([, id]: string[]) => Promise.resolve({} as LobeAgentConfig),
			{
				onSuccess: (data) => {
	...
	useFetchFilesAndKnowledgeBases: () => {
		return useClientDataSWR<KnowledgeItem[]>(
			...
			([, id]: string[]) => Promise.resolve([] as KnowledgeItem[]),
			{
				fallbackData: [],
				suspense: true,
			}
	...
	useInitInboxAgentStore: (isLogin, defaultAgentConfig) =>
		useOnlyFetchOnceSWR<DeepPartial<LobeAgentConfig>>(
			...
			() => Promise.resolve({} as DeepPartial<LobeAgentConfig>),
			{
				onSuccess: (data) => {
					set(
						{
