...
export interface UserSettingsAction {
	...
	updateSystemAgent: (key: UserSystemAgentConfigKey, value: Partial<SystemAgentItem>) => Promise<void>;
}

export const createSettingsSlice: StateCreator<UserStore, [["zustand/devtools", never]], [], UserSettingsAction> = (
	set,
	get
) => ({
	...
	internal_createSignal: () => {
		...
		if (abortController && !abortController.signal.aborted) abortController.abort(MESSAGE_CANCEL_FLAT);
