...
import type { GlobalLLMProviderKey, UserKeyVaults, UserModelProviderConfig } from "@/types/user/settings";
...
export interface ModelListAction {
	...
	dispatchCustomModelCards: (provider: GlobalLLMProviderKey, payload: CustomModelCardDispatch) => Promise<void>;
	/**
	...
	useFetchProviderModelList: (provider: GlobalLLMProviderKey, enabledAutoFetch: boolean) => SWRResponse;
}

export const createModelListSlice: StateCreator<UserStore, [["zustand/devtools", never]], [], ModelListAction> = (
	set,
	get
) => ({
