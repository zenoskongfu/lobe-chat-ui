...
export interface ChatTopicAction {
	...
	internal_createTopic: (params: any) => Promise<string>;
...
export const chatTopic: StateCreator<ChatStore, [["zustand/devtools", never]], [], ChatTopicAction> = (set, get) => ({
		...
		useClientDataSWR<ChatTopic[]>(
			...
			// async ([, sessionId]: [string, string]) => topicService.getTopics({ sessionId }),
			{
				...
				onSuccess: (topics) => {
					...
					set({ topicMaps: nextMap, topicsInit: true }, false, n("useFetchTopics(success)", { sessionId }));
				},
			}
		),
	...
	switchTopic: async (id, skipRefreshMessage) => {
		set({ activeTopicId: !id ? (null as any) : id, activeThreadId: undefined }, false, n("toggleTopic"));
	...
	internal_updateTopicTitleInSummary: (id, title) => {
		get().internal_dispatchTopic({ type: "updateTopic", id, value: { title } }, "updateTopicTitleInSummary");
	},
	refreshTopic: async () => {
