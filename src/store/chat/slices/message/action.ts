...
export interface ChatMessageAction {
	...
	internal_updateMessagePluginError: (id: string, error: ChatMessagePluginError | null) => Promise<void>;
	/**
...
export const chatMessage: StateCreator<ChatStore, [["zustand/devtools", never]], [], ChatMessageAction> = (
	...
	deleteMessage: async (id) => {
		...
		if (message.tools) {
			const toolMessageIds = message.tools.flatMap((tool) => {
				const messages = chatSelectors.activeBaseChats(get()).filter((m) => m.tool_call_id === tool.id);
	...
	addAIMessage: async () => {
		const { internal_createMessage, updateInputMessage, activeTopicId, activeId, inputMessage, messagesMap } =
			get();

		console.log("addAIMessage messageMap:", messagesMap);
		if (!activeId) return;
	...
	internal_dispatchMessage: (payload) => {
		...
		console.log("internal_dispatchMessage messageMap:", nextMap);
