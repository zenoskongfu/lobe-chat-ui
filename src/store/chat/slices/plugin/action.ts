...
import { ChatMessage, ChatMessageError, ChatToolPayload, CreateMessageParams, MessageToolCall } from "@/types/message";
...
export interface ChatPluginAction {
	...
	fillPluginMessageContent: (id: string, content: string, triggerAiMessage?: boolean) => Promise<void>;
	...
	triggerToolCalls: (id: string, params?: { threadId?: string; inPortalThread?: boolean }) => Promise<void>;
	...
	internal_togglePluginApiCalling: (loading: boolean, id?: string, action?: string) => AbortController | undefined;
...
export const chatPlugin: StateCreator<ChatStore, [["zustand/devtools", never]], [], ChatPluginAction> = (set, get) => ({
	...
	invokeBuiltinTool: async (id, payload) => {
		const { internal_togglePluginApiCalling, internal_updateMessageContent, internal_updatePluginError } = get();
	...
	internal_callPluginApi: async (id, payload) => {
		const { internal_updateMessageContent, refreshMessages, internal_togglePluginApiCalling } = get();
		let data: string;

		try {
			const abortController = internal_togglePluginApiCalling(true, id, n("fetchPlugin/start") as string);
