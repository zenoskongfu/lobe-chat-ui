...
import { SEARCH_SEARXNG_NOT_CONFIG, SearchContent, SearchQuery, SearchResponse } from "@/types/tool/search";
...
export interface SearchAction {
	 ...
	 */
	reSearchWithSearXNG: (id: string, data: SearchQuery, options?: { aiSummary: boolean }) => Promise<void>;
	saveSearXNGSearchResult: (id: string) => Promise<void>;
	searchWithSearXNG: (id: string, data: SearchQuery, aiSummary?: boolean) => Promise<void | boolean>;
	toggleSearchLoading: (id: string, loading: boolean) => void;
}

export const searchSlice: StateCreator<ChatStore, [["zustand/devtools", never]], [], SearchAction> = (set, get) => ({
