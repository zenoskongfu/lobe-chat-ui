...
export const chatThreadMessage: StateCreator<ChatStore, [["zustand/devtools", never]], [], ChatThreadAction> = (
	set,
	get
) => ({
	...
	createThread: async ({ message, sourceMessageId, topicId, type }) => {
		...
		return { threadId: THREAD_DRAFT_ID, messageId: THREAD_DRAFT_ID };
	},
		...
		useClientDataSWR<ThreadItem[]>(
			...
			// async ([, topicId]: [string, string]) => threadService.getThreads(topicId),
			{
				...
				onSuccess: (threads) => {
					...
					set({ threadMaps: nextMap, threadsInit: true }, false, n("useFetchThreads(success)", { topicId }));
				},
			}
		),
	...
	internal_updateThreadTitleInSummary: (id, title) => {
		get().internal_dispatchThread({ type: "updateThread", id, value: { title } }, "updateThreadTitleInSummary");
	},
