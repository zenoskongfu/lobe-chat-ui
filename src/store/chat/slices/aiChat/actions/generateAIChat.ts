...
export interface AIGenerateAction {
	 ...
	 */
	internal_toggleChatLoading: (loading: boolean, id?: string, action?: string) => AbortController | undefined;
	/**
	 ...
	 */
	internal_toggleChatReasoning: (loading: boolean, id?: string, action?: string) => AbortController | undefined;
}

export const generateAIChat: StateCreator<ChatStore, [["zustand/devtools", never]], [], AIGenerateAction> = (
	set,
	get
) => ({
	...
	internal_coreProcessMessage: async (originalMessages, userMessageId, params) => {
		...
		const historyCount = chatConfig.historyCount || (DEFAULT_AGENT_CHAT_CONFIG.historyCount as number);
	...
	internal_fetchAIChatMessage: async (messages, messageId, params) => {
		...
		// 4. handle max_tokens
		agentConfig.params.max_tokens = chatConfig.enableMaxTokens ? agentConfig.params.max_tokens : undefined;
