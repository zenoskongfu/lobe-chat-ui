...
export const generalActionSlice: StateCreator<GlobalStore, [["zustand/devtools", never]], [], GlobalGeneralAction> = (
	set,
	get
) => ({
	switchLocale: (locale) => {
	...
	useInitSystemStatus: () =>
		useOnlyFetchOnceSWR<SystemStatus>("initSystemStatus", () => get().statusStorage.getFromLocalStorage(), {
			onSuccess: (status) => {
