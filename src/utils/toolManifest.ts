...
export const convertOpenAIManifestToLobeManifest = (data: OpenAIPluginManifest): LobeChatPluginManifest => {
...
export const getToolManifest = async (url?: string, useProxy: boolean = false): Promise<LobeChatPluginManifest> => {
	// 1. valid plugin
	if (!url) {
 ...
 *
 */
export const convertPluginManifestToToolsCalling = (manifests: LobeChatPluginManifest[]): ChatCompletionTool[] => {
